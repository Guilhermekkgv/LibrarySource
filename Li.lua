...
function Linux:Notify(config)
    ...
    local Notification = Linux.Instance("Frame", {
        ...
        Position = UDim2.new(1, PAPA, 1, -notificationHeight - 10),
        ZIndex = 100
    })
...
function Linux.Create(config)
    ...
    Linux.Instance("UICorner", {
        Parent = Main,
        CornerRadius = UDim.new(0,  onKeyDown)
    })
    ...
    if config.ShowBetaLabel ~= false then
        local BetaLabel = Linux.Instance("Frame", {
            ...
            Position = UDim2.new(0, TitleLabel.TextBounds.X + 15, 0, 9),
            ZIndex = 2
        })
    ...
    local TabsBar = Linux.Instance("Frame", {
        ...
        Size = UDim2.new(0, config.TabWidth or 110, 1, -35 - profileHeight),
        ZIndex = 2,
        BorderSizePixel = 0
    })

    local SearchBar = Linux.Instance("Frame", {
        Parent = TabsBar,
    ...
    local Content = Linux.Instance("Frame", {
        Parent = Main,
        BackgroundTransparency = 1,
        Position = UDim2.new(0, config.TabWidth or 110, 0, 35),
        Size = UDim2.new(1, -(config.TabWidth or 110), 1, -35 - profileHeight),
        ZIndex = 1,
        BorderSizePixel = 2,
    ...
    function LinuxLib.Tab(config)
        ...
        local function SearchElements(query)
            ...
            for _, element in pairs(Elements) do
                local elementFrame = element.Instance
                local shouldShow = query == "" or element.Name:lower():find(query) or element.Type:lower():find(query)
                elementFrame.Visible = shouldShow
            end
        end
        ...
        function TabElements.Button(config)
            ...
            local hoverColor = Color3.fromRGB(128, 0, 128)
            local clickColor = Color3.fromRGB(128, 0, 128)
            ...
            Elements[BtnFrame] = { Instance = BtnFrame, Name = config.Name, Type = "Button" }
        ...
        function TabElements.Toggle(config)
            ...
            Elements[Toggle] = { Instance = Toggle, Name = config.Name, Type = "Toggle" }
        ...
        function TabElements.Dropdown(config)
            ...
            Elements[Dropdown] = { Instance = Dropdown, Name = config.Name, Type = "Dropdown" }
        ...
        function TabElements.Slider(config)
            ...
            Elements[Slider] = { Instance = Slider, Name = config.Name, Type = "Slider" }
        ...
        function TabElements.Input(config)
            ...
            Elements[Input] = { Instance = Input, Name = config.Name, Type = "Input" }
        ...
        function TabElements.Label(config)
            ...
            Elements[LabelFrame] = { Instance = LabelFrame, Name = config.Text or "Label", Type = "Label" }
        ...
        function TabElements.Section(config)
            ...
            Elements[Section] = { Instance = Section, Name = config.Name, Type = "Section" }
        ...
        function TabElements.Paragraph(config)
            ...
            Elements[ParagraphFrame] = { Instance = ParagraphFrame, Name = config.Title or "Paragraph", Type = "Paragraph" }
        ...
        function TabElements.Notification(config)
            ...
            Elements[NotificationFrame] = { Instance = NotificationFrame, Name = config.Name, Type = "Notification" }
