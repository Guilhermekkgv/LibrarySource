...
function Linux.Create(config)
    ...
    local offsetX = TitleLabel.TextBounds.X + 15
    if config.ShowBetaLabel ~= false then
        local BetaLabel = Linux.Instance("Frame", {
            Parent = TopBar,
            BackgroundColor3 = Color3.fromRGB(255, 255, 255),
            Size = UDim2.new(0, 40, 0, 16),
            Position = UDim2.new(0, offsetX, 0, 9.5),
            ZIndex = 2
        })
    ...
    local SearchBox = Linux.Instance("TextBox", {
        ...
        Position = UDim2.new(0, offsetX, 0, 5.5),
    ...
    local function UpdateSearch()
        ...
        for _, tab in pairs(Tabs) do
            for _, element in pairs(tab.Elements) do
                local elementText = element.SearchText:lower()
                element.Instance.Visible = searchText == "" or elementText:find(searchText, 1, true)
            end
        end
    end

    local lastSearchUpdate = 0
    local searchDebounce = 0.1
    SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
        local currentTime = tick()
        if currentTime - lastSearchUpdate >= searchDebounce then
            UpdateSearch()
    ...
    function LinuxLib.Tab(config)
        ...
        local function SelectTab()
            for _, tab in pairs(Tabs) do
                ...
                tab.Button.BorderColor3 = Color3.fromRGB(100, 100, 100)
                tab.Button.BorderTransparency = 0.3
        ...
        function TabElements.Button(config)
            ...
            local hoverColor = Color3.fromRGB(128, 0, 255)
            local clickColor = Color3.fromRGB(128, 0, 255)
            ...
            Tabs[tabIndex].Elements[elementOrder] = {
                Instance = BtnFrame,
        ...
        function TabElements.Input(config)
            ...
            local TextBox = Linux.Instance("TextBox", {
                ...
                PlaceholderColor3 = Color3.fromRGB(150, 150, 150),
                TextColor3 = Color3.fromRGB(180, ಠ_ಠ180, 180),
                TextSize = 14,
                TextScaled = false,
        ...
        function TabElements.Label(config)
            ...
            Tabs[tabIndex].Elements[elementOrder] = {
                Instance = LabelFrame,
                SearchText = config.Text or "Label"
            }
        ...
        function TabElements.Paragraph(config)
            ...
            Tabs[tabIndex].Elements[elementOrder] = {
                ...
                SearchText = config.Title or "Paragraph"
            }
