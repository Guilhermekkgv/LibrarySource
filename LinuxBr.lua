...
function Linux:KeySystem(config, keySettings, onSuccess)
    local isMobile = InputService.TouchEnabled and not InputService.KeyboardEnabled
    ...
    local KeySystemUI = Linux.Instance("ScreenGui", {
        Name = "KeySystemUI",
        Parent = RunService:IsStudio() and LocalPlayer.PlayerGui or game:GetService("CoreGui"),
        ResetOnSpawn = false,
        Enabled = true
    ...
    local Subtitle = Linux.Instance("TextLabel", {
        Parent = KeyFrame,
        BackgroundTransparency = 1,
        Size = UDim2.new(1, -10, 0, 20),
        Position = UDim2.new(0, 5, 0, 45),
    ...
    local Note = Linux.Instance("TextLabel", {
        Parent = KeyFrame,
        BackgroundTransparency = 1,
        Size = UDim2.new(1, -20, 0, 0),
        Position = UDim2.new(0, 10, 0, 70),
        Font = Enum.Font.SourceSans,
        Text = keySettings.Note or "Please enter a valid key to proceed.",
        TextColor3 = Color3.fromRGB(150, 150, 150),
        TextSize = 14,
        TextWrapped = true,
        TextXAlignment = Enum.TextXAlignment.Center,
    ...
    local KeyInput = Linux.Instance("TextBox", {
        Parent = KeyFrame,
        BackgroundColor3 = Color3.fromRGB(25, 25, 25),
        BorderColor3 = Color3.fromRGB(50, 50, 50),
        BorderSizePixel = 2,
        Size = UDim2.new(0.8, 0, 0, 34),
        Position = UDim2.new(0.1, 0, 0, 110 + Note.TextBounds.Y),
    ...
    local ValidateButton = Linux.Instance("TextButton", {
        Parent = KeyFrame,
        BackgroundColor3 = Color3.fromRGB(19, 19, 19),
        BorderColor3 = Color3.fromRGB(50, 50, 50),
        BorderSizePixel = 2,
        Size = UDim2.new(0.4, 0, 0, 34),
        Position = UDim2.new(0.3, 0, 0, 150 + Note.TextBounds.Y),
        Font = Enum.Font.Gotham,
        Text = "Validate",
        TextColor3 = Color3.fromRGB(255, 255, 255),
        TextSize = 14,
    ...
    local ProgressBar = Linux.Instance("Frame", {
        ...
        Position = UDim2.new(0.1, 0, 0, 190 + Note.TextBounds.Y),
        ZIndex = 201,
        BorderSizePixel = 0
    })
    ...
    local SecondActionButton
    if keySettings.SecondAction and keySettings.SecondAction.Enabled then
        SecondActionButton = Linux.Instance("TextButton", {
            Parent = KeyFrame,
            BackgroundColor3 = Color3.fromRGB(19, 19, 19),
            BorderColor3 = Color3.fromRGB(50, 50, 50),
            BorderSizePixel = 2,
            Size = UDim2.new(0.4, 0, 0, 34),
            Position = UDim2.new(0.3, 0, 0, 230 + Note.TextBounds.Y),
            Font = Enum.Font.Gotham,
            Text = keySettings.SecondAction.Type == "Discord" and "Copy Discord Invite" or "Get Key",
            TextColor3 = Color3.fromRGB(255, 255, 255),
            TextSize = 14,
            ZIndex = 201,
            AutoButtonColor = false
    ...
    local function ApplyButtonAnimations(button)
    ...
    ApplyButtonAnimations(ValidateButton)
    if SecondActionButton then
        ApplyButtonAnimations(SecondActionButton)
    end

    ValidateButton.MouseButton1Click:Connect(function()
        local key = KeyInput.Text
        local isValid = table.find(keySettings.Key, key)

        local progressTween = TweenInfo.new(1, Enum.EasingStyle.Linear, Enum.EasingDirection.In)
        TweenService:Create(ProgressFill, progressTween, {Size = UDim2.new(1, 0, 1, 0)}):Play()
        ...
        if isValid then
            if keySettings.SaveKey then
                ...
                if keySettings.SaveInRoot then
                    Linux:Notify({
                        Title = "Key System",
                        Content = "Key saved to clipboard. Configure external storage (e.g., Pelican, Luarmor).",
                        Duration = 5
                    })
                else
                    Linux:Notify({
                        Title = "Key System",
                        Content = "Key saved to clipboard.",
                        Duration = 5
                    })
                end
            end
    ...
    if SecondActionButton then
        SecondActionButton.MouseButton1Click:Connect(function()
            local parameter = keySettings.SecondAction.Parameter
            local link = keySettings.SecondAction.Type == "Discord" and "discord.com/invite/" .. parameter or parameter
            setclipboard(link)
            Linux:Notify({
                Title = "Key System",
                Content = "Link copied to clipboard: " .. link,
                Duration = 5
            })
        end)
    end
...
function Linux.Create(config)
    ...
    local function CreateMainUI()
        ...
        local ExecutorLabel
        if config.IdentifyExecutor then
            local executorName = identifyexecutor and identifyexecutor() or "Invalid Executor"
            if executorName ~= "Invalid Executor" then
                ExecutorLabel = Linux.Instance("Frame", {
                    Parent = TopBar,
                    BackgroundColor3 = Color3.fromRGB(28, 28, 28),
                    Size = UDim2.new(0, 80, 0, 16),
                    Position = UDim2.new(0, TitleLabel.TextBounds.X + (config.ShowBetaLabel ~= false and 60 or 15), 0, 4),
                    ZIndex = 2
                })
        ...
        function LinuxLib.Tab(config)
            ...
            function TabElements.Toggle(config)
                ...
                local function UpdateToggle()
                    ...
                    if State then
                        ...
                        ToggleGradient = Linux.Instance("UIGradient", {
                            Parent = ToggleFill,
                            Color = ColorSequence.new({
                                ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 120, 255)),
                                ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 0, 139))
            ...
            function TabElements.Paragraph(config)
                ...
                local function SetText(title, content)
                    ParagraphFrame:GetChildren()[1].Text = tostring(title)
        ...
        if config.KeySystem then
            Linux:KeySystem(config, config.KeySettings, function()
                LinuxUI.Enabled = true
                CreateMainUI()
            end)
        else
            LinuxUI.Enabled = true
