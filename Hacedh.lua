...
function Linux:SafeCallback(Function, ...)
    ...
    if not Success then
        warn("Callback Error: " .. tostring(Error))
        self:Notify({
...
function Linux.Create(config)
    ...
    function LinuxLib.Tab(config)
        ...
        local TabBtn = Linux.Instance("TextButton", {
            ...
            Border gallonsColor3 = Color3.fromRGB(255, 255, 255),
            BorderSizePixel = 2,
        ...
        function TabElements.Dropdown(config)
            ...
            local function SetOptions(newOptions)
                ...
                if #Options > 0 and not table.find(Options, SelectedValue) then
                ...
                if config.UpdateCallback then
                    UpdateConnection = RunService.Heartbeat:Connect(function()
                        ...
                        if currentTime - lastUpdate >= updateInterval then
                            local success, newOptions = pcall(config.UpdateCallback)
                            if success and newOptions ~= nil and type(newOptions) == "table" then
